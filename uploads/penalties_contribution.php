...
try {
    // Fetch the total count of missed contributions
    ...
    // Fetch missed contributions with pagination
    $stmt = $pdo->prepare("
        SELECT id, penalty_amount, infraction_date,reason,missed_contribution_date
        FROM penalties 
        WHERE user_id = :user_id AND tontine_id = :tontine_id
        ORDER BY missed_contribution_date DESC
        LIMIT :start, :perPage
    ");
            ...
            <tr>
                <td><?php echo $counter++; ?></td> <!-- Increment the counter -->
                            ...
                            <td><?php echo htmlspecialchars($contribution['reason']); ?></td>
                                 <td><?php echo htmlspecialchars($contribution['missed_contribution_date']); ?></td>
